...
int bin_ser(Node *p, int k)
{
    if (p == NULL) //line 1 because if p is null u cant check p->data as it doesnt exist
        return 0;
    if (p->data == k)
        return 1;
    if (p->data > k)
        return bin_ser(p->lchild, k);
    if (p->data < k)
        return bin_ser(p->rchild, k);
}